...
You can also view all the data you have downloaded with a simple command `prism_archive_ls()`.  By default this just gives a list of file names.  All the internal functions in the package work off of this simple list of files.

```{r listing files, out.lines=10}
## Truncated to keep file list short
prism_archive_ls()
```

While internal plotting functions use this, other files may want an absolute path (e.g. the `raster` package). The `pd_to_file` function conveniently returns the absolute path.  Alternatively you may want to see what the normal name for the product is (not the file name), and we can get that with the `pd_get_name` function.
```{r more listing, out.lines=10}
## Truncated to keep file list short
...
You can easily make a quick plot of your data to using the output of `prism_archive_ls()`

```{r quick_plot,fig.height=5,fig.width=7}
pd_image(prism_archive_ls()[1])
```
...
to_slice <- grep("_[0-9]{4}[0][1]", prism_archive_ls(), value=T)
...
all_prism_files <- pd_to_file(prism_archive_ls())
jnorm <- all_prism_files[match(
  jnorm_name, 
  tools::file_path_sans_ext(basename(all_prism_files))
  )]
j2013 <- all_prism_files[match(
  j2013_name, 
  tools::file_path_sans_ext(basename(all_prism_files))
  )]
...
## Grab the data, pd_to_file(prism_archive_ls()) will show you all the data you've downloaded
norm <- grep("tmean_30yr_normal_4kmM2_annual", pd_to_file(prism_archive_ls()), value=T)
rast <- raster(norm)
